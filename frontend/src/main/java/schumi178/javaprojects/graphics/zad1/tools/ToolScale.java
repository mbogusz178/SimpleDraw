...
public class ToolScale implements Tool {
    ...
    public ToolScale(BiConsumer<Double, Double> updateOriginPoint) {
    ...
    public boolean onLeftDragged(ToolUseEvent event) {
        if(currentShape != null) {
            ...
            MouseEvent mouseEvent = event.getMouseEvent();
            currentMouseLocationX = mouseEvent.getX();
            if(lastMouseLocationX != 0 && currentMouseLocationX != 0) {
                double xDiff = currentMouseLocationX / lastMouseLocationX;
                currentShape.scaleByFactor(xDiff, new Point2D(originX, originY));
                return true;
            }
        }
        return false;
